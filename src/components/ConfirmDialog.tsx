...
export function ConfirmDialog({ title, confirmText = '确认', onConfirm, onCancel }: ConfirmDialogProps) {
  return (
    <div className="fixed inset-0 bg-black/30 grid place-items-center z-50" onClick={onCancel} role="dialog" aria-modal="true">
      <div className="bg-[var(--color-canvas)] rounded-xl p-5 min-w-[280px] shadow-lg" onClick={(e) => e.stopPropagation()}>
        <p className="text-[15px] mb-4">{title}</p>
        <div className="flex justify-end gap-2">
          <button onClick={onCancel} className="px-3 py-1.5 rounded-lg text-[13px] hover:bg-[var(--color-surface-2)]">取消</button>
