...
export function LogDrawer() {
  ...
  return (
    <div className="shrink-0 border-t border-[var(--color-border)]" style={open ? { height } : {}}>
      ...
      <div className="flex items-center gap-2 px-4 h-7 text-[12px]">
        ...
        <span className="text-[var(--color-text-muted)]">后端日志</span>
        {!open && last && <span className="text-[var(--color-text-muted)] truncate flex-1">{last.line}</span>}
        <span className="text-[var(--color-text-muted)]">{entries.length}</span>
        <button onClick={clear} className="p-1 rounded hover:bg-[var(--color-surface-2)]" aria-label="清空"><Trash size={13} /></button>
        <button onClick={() => setOpen((o) => !o)} className="p-1 rounded hover:bg-[var(--color-surface-2)]" aria-label={open ? '收起' : '展开'}>
      ...
      {open && (
        <div ref={scrollRef} onScroll={(e) => {
          ...
          {entries.length === 0 ? <div className="text-[var(--color-text-muted)] py-4">暂无日志，sidecar 启动后此处显示模型调用输出</div> :
            entries.map((e, i) => {
              ...
              return <div key={i} className={err ? 'text-[var(--color-accent)]' : 'text-[var(--color-text-muted)]'}><span className="opacity-60">{ts}</span> {e.line}</div>
            })}
          {!follow && <button onClick={() => setFollow(true)} className="sticky bottom-2 ml-auto block px-2 py-1 rounded bg-[var(--color-surface-2)] text-[var(--color-text)]" aria-label="跟随最新">↓ 跟随最新</button>}
        </div>
      )}
    </div>
  )
